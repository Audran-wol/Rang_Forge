...
export async function generateMetadata({ params }: { params: { country: string } }) {
  // Format country name properly for display
  ...
  return {
    ...
    description: `Discover the most talented GitHub developers from ${countryName}. View comprehensive rankings based on followers, contributions, repositories, and community impact. Updated ${new Date().toLocaleDateString()}.`,
    keywords: [
      ...
      `github ranking ${countryName.toLowerCase()}`,
      `developer leaderboard ${countryName.toLowerCase()}`,
      `software engineers ${countryName.toLowerCase()}`,
      `${countryName.toLowerCase()} coding talent`,
      `top programmers from ${countryName.toLowerCase()}`
    ],
    alternates: {
      canonical: `https://rang-forge.netlify.app/ranking/${params.country.toLowerCase()}`
    },
    openGraph: {
      title: `Top GitHub Developers in ${countryName} | Developer Rankings ${new Date().getFullYear()}`,
      description: `Explore the most influential GitHub developers in ${countryName}. Comprehensive rankings based on followers, contributions, and community impact. Find the best developer talent in ${countryName}.`,
      ...
      images: [
        {
          url: `/og-images/${params.country.toLowerCase()}.png`,
          alt: `Top GitHub Developers in ${countryName}`,
          width: 1200,
          height: 630,
        }
      ]
    ...
    twitter: {
      ...
      title: `Top GitHub Developers in ${countryName} | Rang Forge`,
      description: `Discover the best GitHub developers from ${countryName}. View rankings by contributions and impact.`,
      images: [`/og-images/${params.country.toLowerCase()}.png`],
    }
  };
}
...
export default function CountryPage() {
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      // Use dynamic import for the country-specific data
      const users = require(`/country_files/${countrySlug.toLowerCase()}.json`);
      setState(prev => ({
        ...prev,
  ...
  const jsonLd = {
    ...
    "description": `Comprehensive rankings of the best GitHub developers from ${countryName} based on followers, contributions, and community impact.`,
    "itemListElement": filteredUsers.slice(0, 10).map((user, index) => ({
      "@type": "ListItem",
      "position": index + 1,
      "item": {
        "@type": "Person",
        ...
        "url": `https://rang-forge.netlify.app/ranking/${countrySlug}/${user.login}`,
